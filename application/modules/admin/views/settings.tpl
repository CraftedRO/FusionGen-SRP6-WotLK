	...
	<section class="box big">
		...
		<form onSubmit="Settings.saveWebsiteSettings(); return false">
			...
			<select id="disabled_expansions">
				<option value="wotlk" {if count($config.disabled_expansions) == 0}selected{/if}>Wrath of the Lich King</option>
				<option value="tbc" {if count($config.disabled_expansions) == 1}selected{/if}>The Burning Crusade</option>
				<option value="none" {if count($config.disabled_expansions) == 2}selected{/if}>No expansion allowed</option>
			</select>
