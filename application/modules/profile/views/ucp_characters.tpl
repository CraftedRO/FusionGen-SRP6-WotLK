	...
	{foreach from=$realms item=realm}
		{if $realm->getCharacterCount($id) > 0}
			...
			{foreach from=$realm->getCharacters()->getCharactersByAccount($id) item=character}
				<a href="{$url}character/{$realm->getId()}/{$character.name}" data-tip="<img src='{$url}application/images/stats/{$character.class}.gif' align='absbottom'/>&nbsp;&nbsp;{$character.name} (Lvl {$character.level})">
